...
void ScaleRotation::OnHoldingRotation(int x, int y)
{
	...
	if (selectedTool == 1){
		...
		if (type > 0){
			...
			float angy = (to.x - firstmove.x) + oldAngle.y;// zmieniony plus na minus by nie trzeba było 
...
{
	if (hasShift){
		//zamieniamy te wartości by przesuwanie w osi x działało poprawnie, a nie na odwrót.
		//drgawki nawet w photoshopie występują bo wtedy jedna oś ma + druga - i w zależności od tego która jest użyta
		//zwiększa bądź zmniejsza nam tekst/rysunek.
