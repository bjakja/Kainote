...
void SubsGrid::SetStyle() 
{
	...
	font.SetPointSize(Options.GetInt(_T("Grid Font Size")));
...
void SubsGrid::DrawGrid(wxDC &mdc,int w, int h)
{
	...
	wxColour labelBkCol=Options.GetColour(_T("Grid Label Saved"));
	wxColour labelBkColN=Options.GetColour(_T("Grid Label Normal"));
	wxColour labelBkColM=Options.GetColour(_T("Grid Label Modified"));
	wxColour linesCol=Options.GetColour(_T("Grid Lines"));
	wxColour subsBkCol=Options.GetColour(_T("Grid Dialogue"));
	wxColour comm=Options.GetColour(_T("Grid Comment"));
	wxColour seldial=Options.GetColour(_T("Grid Selected Dialogue"));
	wxColour selcom=Options.GetColour(_T("Grid Selected Comment"));
	wxColour textcol=Options.GetColour(_T("Grid Text"));
	wxColour collcol=Options.GetColour(_T("Grid Collisions"));
	...
	Dialogue *acdial=GetDial(MID(0,Edit->ebrow,GetCount()));
	Dialogue *Dial;

	int panelrows=(h/(GridHeight+1))+1;
	int scrows;
	if((scPos+panelrows)>=GetCount()+2){
		scrows=GetCount()+1;bg=true;scPos=(scrows-panelrows)+1;
		if(panelrows>GetCount()+1){scPos=0;}
	}
	else{
		scrows=(scPos+panelrows);
	}
	SetScrollbar(wxVERTICAL,scPos,panelrows,GetCount()+2);
	int fw,fh,bfw,bfh;
	...
	for(int i=scPos;i<scrows;i++){
		...
		if (i==scPos){
			...
			strings.Add(wxString::Format(_T("%i"),i));

			isd=Dial->IsComment;
			states=Dial->State;
			if (form<SRT){
			strings.Add(wxString::Format(_T("%i"),Dial->Layer));
			}
			...
			if (form<SRT){
				...
				strings.Add(wxString::Format(_T("%i"),Dial->MarginL));
				strings.Add(wxString::Format(_T("%i"),Dial->MarginR));
				strings.Add(wxString::Format(_T("%i"),Dial->MarginV));
				strings.Add(Dial->Effect);
			}
			if(form!=TMP && !(CNZ & visible)){
				int chtime=CalcChars((!showtl&&transl&&Dial->TextTl!="")? Dial->TextTl : Dial->Text) / ((Dial->End.mstime-Dial->Start.mstime)/1000.0f);
				if(chtime<0){chtime=999;}
				strings.Add(wxString::Format(_T("%i"),chtime));
				shorttime=chtime>15;
			}else if(form!=TMP){strings.Add("");}
			//errors=Dial->spells;
		...
		for (int j=0; j<ilcol; j++){
			...
			if(GridWidth[j]>0){
				...
				if(unkstyle && j==4 || shorttime && (j==10||(j==3&&form>SSA))){dc.SetBrush(wxBrush(Options.GetColour("Grid Spellchecker")));}

				dc.DrawRectangle(posX,posY,GridWidth[j],GridHeight);
				
				if(j==ilcol-1 && errors.size()>1){
					
					for(size_t k = 0; k < errors.size(); k+=2){
						
						wxString err=strings[j].SubString(errors[k], errors[k+1]);
						err.Trim();
						if(errors[k]>0){
							wxString berr=strings[j].Mid(0, errors[k]);
				...
				if(j==ilcol-1 && (strings[j].StartsWith("T") || strings[j].StartsWith("Y") || strings[j].StartsWith("£"))){posX++;}
	...
	if(bg){
		...
		tdc.SetPen(*wxTRANSPARENT_PEN);
		tdc.SetBrush(wxBrush(Options.GetColour(_T("Grid Background"))));
		tdc.DrawRectangle(0,posY,w+scHor-GridWidth[0]+1,h);
	}
	if(mtimerow>=scPos&&mtimerow<=scrows){
		 mdc.SetBrush(*wxTRANSPARENT_BRUSH);
		 mdc.SetPen(wxPen(Options.GetColour(_T("Grid Active Line")),3));
	...
	if(Edit->ebrow>=scPos&&Edit->ebrow<=scrows){
		 ...
		 tdc.SetPen(wxPen(Options.GetColour(_T("Grid Active Line"))));
...
void SubsGrid::AdjustWidths(int cell)
{
	...
	for(int i=0;i<maxx;i++)
	{
		...
		if(form<SRT){
			if((LAYER & cell) && ndial->Layer!=0){
				dc.GetTextExtent(wxString::Format(_T("%i"),ndial->Layer), &fw, &fh, NULL, NULL, &font);
	...
	if(form>SSA){GridWidth[4]=0;GridWidth[5]=0;GridWidth[6]=0;GridWidth[7]=0;GridWidth[8]=0;GridWidth[9]=0;}
...
void SubsGrid::ScrollTo(int y, bool center){
	...
	int nextY = MID(0,y,GetCount()+1 - h/(GridHeight+1));
...
void SubsGrid::OnMouseEvent(wxMouseEvent &event) {
	...
	if (holding) {
		// Find direction
		int minVis = scPos+1;
		int maxVis = scPos+h/(GridHeight+1)-3;
		int delta = 0;
		if (row < minVis && row!=0) delta = -1;
		if (row > maxVis) delta = +1;

		if (delta) {
			ScrollTo(MID(row - (h / (GridHeight+1)), scPos + delta, row));
...
void SubsGrid::Convert(char type)
{
	...
	if(type==ASS){
		...
		if(resx==""){resx="1280";}
		if(resy==""){resx="720";}
		AddSInfo(_T("PlayResX"),resx, false);
		AddSInfo(_T("PlayResY"),resy, false);
...
void SubsGrid::ChangeTime()
{
	...
	int tim = Options.GetInt(_T("Change Time"));
	int lmd = MAX(0,Options.GetInt(_T("Change mode")));
	...
	bool fromsel=false,fromstyl=false;
	...
	wxStringTokenizer styles(style,_T(";"));
	...
	int czas1=GetDial(fs)->Start.mstime;
	Dialogue *dialc;
	tim+=added;
	for (int i=0;i<GetCount();i++)
	{
		...
		if(lmd==3){
		fromstyl=(czas1<=file->subs->dials[i]->Start.mstime);
		}
		if((i>=fs&&lmd==2)||lmd==0){fromsel=true;}
		if((sel.find(i) != sel.end() && lmd==1) || fromsel || fromstyl){
			...
			if(CT>0 || pe){
				...
				if(CT>0 || pe>3){
					tmpmap.erase(GetDial(i));
					tmpmap[dialc]=i;
					if(i>0){
						auto it = tmpmap.find(GetDial(i-1));
						it++;
						if(CT>0 && file->subs->dials[i-1]->End > it->first->Start){
							file->subs->dials[i-1]->End = it->first->Start;
							file->subs->dials[i-1]->State=1;
						}
						if(pe & 4){
							int cdiff= (te+ts);
							int tdiff = it->first->Start.mstime - file->subs->dials[i-1]->End.mstime;
							if(tdiff <= cdiff){
								float wsp = ((float)tdiff / (float)cdiff)*te;
								file->subs->dials[i-1]->End.Change(ZEROIT(wsp));
								dialc->Start=file->subs->dials[i-1]->End;
								file->subs->dials[i-1]->State=1; dialc->State=1;
							}

						}
						if(pe & 8){
							int strtrng = file->subs->dials[i-1]->Start.mstime-kbs;
							int strtrng1 = file->subs->dials[i-1]->Start.mstime+kas;
							int endrng = file->subs->dials[i-1]->End.mstime-kbe;
							int endrng1 = file->subs->dials[i-1]->End.mstime+kae;
							int pors = 0;
							int pore = it->first->Start.mstime+kas;
							if(i>1){
								it--;it--;
							...
							for (unsigned int g=0;g<vb->VFF->KeyFrames.Count();g++) {
								...
								if (keyMS > strtrng && keyMS < strtrng1) {if(strtres>keyMS){strtres = keyMS;}}
								if (keyMS > endrng && keyMS < endrng1) {if(endres<keyMS){endres = keyMS;}}
							}
							if(strtres!=INT_MAX && strtres >= pors){
								strtres-=halfframe;
								file->subs->dials[i-1]->Start.NewTime(ZEROIT(strtres));
								file->subs->dials[i-1]->State=1;
							}
							if(endres!=-1 && endres <= pore){
								endres-=halfframe;
								file->subs->dials[i-1]->End.NewTime(ZEROIT(endres));
								file->subs->dials[i-1]->State=1;
							}
						}
...
{
	if(file->IsNotSaved()){
		if(file->Iter()<1||!Modified){Modified=true;Kai->MenuBar->Enable(ID_UNDO1,true); 
		Kai->Toolbar->UpdateId(ID_UNDO1,true);
		}
...
void SubsGrid::CheckText(wxString text, wxArrayInt &errs)
{
	...
	if(Kai->SC){
		...
		if(errs.size()<1){errs.push_back(0);}
		//wxMessageBox(deb);
	}
}
...
//	int an=2;
//	bool haspos=false;
//	wxRegEx regan(_T("\\\\an([0-9])"),wxRE_ADVANCED|wxRE_ICASE);
//	wxRegEx reg(_T("\\\\pos\\(([^,]*)\\,([^\\)]*)"),wxRE_ADVANCED|wxRE_ICASE);
//	wxRegEx regm(_T("\\\\move\\(([^,]*)\\,([^,]*)\\,([^,]*)\\,([^\\)]*)"),wxRE_ADVANCED|wxRE_ICASE);
...
//	}
//
//	wxRegEx regst(_T("\\{[^\\{]*\\}"),wxRE_ADVANCED);
...
{
	wxString oldx=GetSInfo(_T("PlayResX"));
	wxString oldy=GetSInfo(_T("PlayResY"));
	int nx=wxAtoi(oldx);
	int ny=wxAtoi(oldy);
...
BEGIN_EVENT_TABLE(SubsGrid,wxWindow)
	...
	//EVT_COMMAND_SCROLL(idscroll,SubsGrid::OnScroll)
