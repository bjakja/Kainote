...
ffmpeg2::ffmpeg2(wxString _filename, int tab, bool *success)
	{
	Cache=0;
	blnum=0;
...
bool ffmpeg2::LoadFromClip(AVSValue _clip)
	{
	// Prepare avisynth
	AVSValue script;
...
void ffmpeg2::GetAudio(void *buf, int64_t start, int64_t count)
	{
...
void ffmpeg2::GetBuffer(void *buf, int64_t start, int64_t count)
	{
	wxMutexLocker lock(AviSynthMutex);
