...
FontCollectorDialog::FontCollectorDialog(wxWindow *parent)
	: wxDialog(parent,-1,"Kolekcjoner czcionek",wxDefaultPosition, wxDefaultSize, wxDEFAULT_DIALOG_STYLE|wxRESIZE_BORDER)
{
	...
	choosepath=new wxButton(this,8799,"Wybierz folder");
	...
	choices.Add("SprawdŸ dostêpnoœæ czcionek");
	choices.Add("Skopiuj do wybranego folderu");
	choices.Add("Spakuj zipem");
	opts=new wxRadioBox(this,9987,"Opcje",wxDefaultPosition,wxDefaultSize,choices,0,wxRA_SPECIFY_ROWS);
	...
	subsdir=new wxCheckBox(this,7998,"Zapisuj do folderu z napisami");
	subsdir->Enable(Options.GetInt("Font Collect Action")!=0);
	subsdir->SetValue(Options.GetBool("Collector Subs Directory"));
	

	fromMKV=new wxCheckBox(this,7991,"Wyci¹gnij czcionki z wczytanego pliku MKV");
	...
	bok=new wxButton(this,9879,"Rozpocznij");
	bcancel=new wxButton(this,wxID_CANCEL,"Zamknij");
...
wxArrayString FontCollectorDialog::GetAssFonts(std::vector<bool> &founded, bool check)
{
		...
		while(true)
		{
			...
			if(flet!=-1)
			{
				...
				else if(fonts.Index(fn,false)==-1)
				{
					...
					wxString kkk;
					kkk<<(i+1);
					console->AppendText("Znaleziono czcionkê o nazwie \""+fn+"\" w linii nr "+kkk+".\n \n");
					console->AppendText("Czcionka \""+fn+"\" jest zainstalowana.\n \n");
					founded.push_back(check);
				}
	...
	for(auto cur=notfindfonts.begin(); cur!=notfindfonts.end(); cur++){
		...
		wxString result="Nie mo¿na znaleŸæ czcionki \""+cur->first+"\".\r\n";
		...
		while(token.HasMoreTokens()){
			...
			if(tkn.IsNumber()){
				result+="U¿ytej w linijkach: "+list.Mid(token.GetPosition()-tkn.Len());
				break;
			}else{
				if(first){result+="U¿ytej w stylach:\r\n";first=false;}
				result+=" - "+tkn+"\r\n";
			}
		}
...
void FontCollectorDialog::CopyFonts(bool check)
{
	...
	{wxLogMessage("dupa blada freetype nie chce siê zinitializowaæ\n");return;}
	bool allglyphs=true;
	...
	console->SetDefaultStyle(wxTextAttr(*wxBLACK));
	console->AppendText("Rozpoczêto szukanie czcionek.\n \n");
	bool notfound=false;
			...
			if(ext=="fon" || ext=="pfb"){
				...
				if( foundedfaces.Index(all,false)==-1 && fontnames.Index(fontfiles[i],false)==-1){
						...
						if(res!=""){
							allglyphs=false;
							console->SetDefaultStyle(wxTextAttr(*wxRED));
							console->AppendText("Czcionka \""+all+"\" nie ma znaków: "+res);
						}
						fontnames.Add(fontfiles[i]);
	...
	if(check){
		...
		if(nfound||!allglyphs){
			console->SetDefaultStyle(wxTextAttr(*wxRED));
			console->AppendText(wxString::Format("Zakoñczono, znaleziono %i czcionek.\nNie znaleziono %i czcionek. %s",  found, nfound, (allglyphs)? L"" : L"\nCzcionka nie ma wszystkich znaków u¿ytych w tekœcie"));}
		else{
			console->SetDefaultStyle(wxTextAttr("#008000"));
			console->AppendText(wxString::Format("Zakoñczono powodzeniem, znaleziono %i czcionek.", found));
		}
		fontfaces.clear();
		foundedfaces.clear();
		fontfiles.clear();
	...
	if(opts->GetSelection()==1){
		...
		for(size_t i=0; i< fontnames.size(); i++)
		{
			...
			if(wxCopyFile(fontnames[i],copypath+"\\"+fontnames[i].AfterLast('\\'))){
				console->SetDefaultStyle(wxTextAttr(*wxBLACK));
				console->AppendText("Skopiowano czcionkê o nazwie \""+fontnames[i].AfterLast('\\')+"\".\n \n");
			}
			else
			{
				console->SetDefaultStyle(wxTextAttr(*wxRED));
				console->AppendText("Nie mo¿na skopiowaæ czcionki o nazwie \""+fontnames[i].AfterLast('\\')+"\".\n \n");
			}
		}
	}
	else
	{
		if(!wxDir::Exists(copypath.BeforeLast('\\')))
		{
			...
			if(wxMessageBox("Plik zip ju¿ istnieje, usun¹æ go?","Potwierdzenie",wxYES_NO,this)==wxYES){wxRemoveFile(copypath);}}
		...
		for(size_t i=0; i< fontnames.size(); i++)
		{
			...
			{
				console->SetDefaultStyle(wxTextAttr(*wxRED));
				console->AppendText("Nie mo¿na spakowaæ czcionki o nazwie \""+fontnames[i].AfterLast('\\')+"\".\n \n");
				founded[i]=false;
			}
	...
	if(nfound){
		console->SetDefaultStyle(wxTextAttr(*wxRED));
		console->AppendText(wxString::Format("Zakoñczono, skopiowano %i czcionek.\nNie uda³o siê skopiowaæ %i czcionek.", (int)fontnames.size(), nfound));}
	else{
		console->SetDefaultStyle(wxTextAttr("#008000"));
		console->AppendText(wxString::Format("Zakoñczono powodzeniem, skopiowano %i czcionek.", (int)fontnames.size()));
	}
}
...
void FontCollectorDialog::CopyMKVFonts()
{
	...
	if(names.size()<1){
		console->SetDefaultStyle(wxTextAttr(*wxRED));
		console->AppendText("Wczytany plik mkv nie ma ¿adnych czcionek.");
		return;}
	...
	else
	{
		...
		for(size_t i=0; i<names.size(); i++){
			...
			{
				console->SetDefaultStyle(wxTextAttr(*wxBLACK));
				console->AppendText("Zapisano czcionkê o nazwie \""+names[i]+"\".\n \n");
			}
			else
			{
				console->SetDefaultStyle(wxTextAttr(*wxRED));
				console->AppendText("Nie mo¿na zapisaæ czcionki o nazwie \""+names[i]+"\".\n \n");
				cpfonts--;
			}
		}
		if(zip)
	...
	if(cpfonts<names.size()){
		...
		console->SetDefaultStyle(wxTextAttr("#008000"));
		console->AppendText(wxString::Format("Zakoñczono powodzeniem, skopiowano %i czcionek.", cpfonts));
	}
...
void FontCollectorDialog::OnButtonStart(wxCommandEvent &event)
{
	...
	{
		if(path->GetValue()==""&& !subsdir->GetValue()){wxMessageBox("Wybierz folder gdzie maj¹ zaostaæ skopiowane czcionki");goto done;}
		if(opts->GetSelection()==2 && !path->GetValue().EndsWith(".zip") && !subsdir->GetValue()){wxMessageBox("Wybierz nazwê dla archiwum");goto done;}
...
{
	if(opts->GetSelection()==1){
		destdir = wxDirSelector("Wybierz folder zapisu", path->GetValue(),0,wxDefaultPosition,this);
	}
	else
	{
		destdir= wxFileSelector("Wybierz nazwê archiwum", (path->GetValue().EndsWith("zip"))? path->GetValue().BeforeLast('\\') : path->GetValue(), 
			(path->GetValue().EndsWith("zip"))? path->GetValue().AfterLast('\\') : "",
			"zip","Pliki archiwum (*.zip)|*.zip",wxFD_SAVE|wxFD_OVERWRITE_PROMPT,this);
	}
...
wxString FontCollectorDialog::CheckChars(FT_Face face, std::map<wxUniChar, wxString> *chars)
{
	...
	for(auto cur=chars->begin(); cur!= chars->end(); cur++)
	{
		...
		if(haschar==0){
			rettxt+= _("\"") + cur->first+ _("\", który wystêpuje w linijkach\n") +cur->second+ _(".\n");
