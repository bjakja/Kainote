...
DShowPlayer::DShowPlayer(VideoCtrl* _parent) :
VideoPlayer(_parent),
m_pGraph(NULL),
m_pControl(NULL),
m_pSeek(NULL),
...
{
	HRESULT hr = S_OK;
	if (vstate == None || !m_pGraph){ return; }
...
{
	if (vstate != Playing && vstate != Paused)
	{
		return;
	}
	HRESULT hr;
...
void DShowPlayer::SetVolume(int volume)
{
	HRESULT hr;
	if (m_pBA)
	{
...
void DShowPlayer::RecreateSurface()
{
