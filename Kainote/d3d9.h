...
constexpr HRESULT D3DERR_DEVICEREMOVED = -2005530512L;
constexpr HRESULT D3DERR_DEVICEHUNG = -2005530508L;
...
constexpr int D3DPOOL_SYSTEMMEM = 2;
constexpr D3DTEXTUREFILTERTYPE D3DTEXF_NONE = 0;
constexpr int D3DSAMP_ADDRESSU = 1, D3DSAMP_ADDRESSV = 2, D3DSAMP_MAGFILTER = 5, D3DSAMP_MINFILTER = 6;
constexpr DWORD D3DTADDRESS_CLAMP = 3;
constexpr int D3DBLEND_ONE = 2;
...
constexpr int D3DCULL_NONE=0, D3DZB_FALSE=0, D3DBLEND_SRCALPHA=5, D3DBLEND_INVSRCALPHA=6;
...
struct D3DReleasable { int refs = 1; ULONG AddRef(){ return ++refs; } ULONG Release(){ if (--refs == 0) { delete this; return 0; } return refs; } };
...
struct IDirect3DSwapChain9 : D3DReleasable { virtual ~IDirect3DSwapChain9() = default; HRESULT Present(const RECT*, const RECT*, HWND, const void*, DWORD){return S_OK;} HRESULT GetBackBuffer(UINT, int, IDirect3DSurface9** out){ if(out)*out=new IDirect3DSurface9(); return S_OK;} };
// Direct3D does not draw on Linux; these calls only have to compile, and textures are never made
struct IDirect3DDevice9 : D3DReleasable { virtual ~IDirect3DDevice9() = default; HRESULT CreateRenderTarget(UINT, UINT, D3DFORMAT, int, DWORD, BOOL, IDirect3DSurface9** out, void*){ if(out)*out=new IDirect3DSurface9(); return S_OK;} HRESULT SetRenderTarget(DWORD, IDirect3DSurface9*){return S_OK;} HRESULT SetViewport(const D3DVIEWPORT9*){return S_OK;} HRESULT CreateTexture(UINT, UINT, UINT, DWORD, D3DFORMAT, int, IDirect3DTexture9** out, void*){ if(out)*out=nullptr; return E_FAIL;} HRESULT UpdateTexture(IDirect3DTexture9*, IDirect3DTexture9*){return E_FAIL;} HRESULT SetTexture(DWORD, IDirect3DTexture9*){return S_OK;} HRESULT SetSamplerState(DWORD, int, DWORD){return S_OK;} HRESULT CreateAdditionalSwapChain(D3DPRESENT_PARAMETERS*, IDirect3DSwapChain9** out){ if(out)*out=nullptr; return E_FAIL;} HRESULT EndScene(){return S_OK;} HRESULT BeginScene(){return S_OK;} HRESULT Present(const RECT*, const RECT*, HWND, const void*){return S_OK;} HRESULT DrawPrimitiveUP(int, UINT, const void*, UINT){return S_OK;} HRESULT StretchRect(IDirect3DSurface9*, const RECT*, IDirect3DSurface9*, const RECT*, int){return S_OK;} HRESULT CreateOffscreenPlainSurface(UINT, UINT, D3DFORMAT, int, IDirect3DSurface9** out, void*){ if(out)*out=new IDirect3DSurface9(); return S_OK;} HRESULT SetFVF(DWORD){return S_OK;} HRESULT TestCooperativeLevel(){return S_OK;} HRESULT Clear(DWORD, const void*, DWORD, D3DCOLOR, float, DWORD){return S_OK;} HRESULT Reset(D3DPRESENT_PARAMETERS*){return S_OK;} HRESULT SetRenderState(int, DWORD){return S_OK;} HRESULT SetTextureStageState(DWORD, int, DWORD){return S_OK;} HRESULT SetTransform(int, const void*){return S_OK;} HRESULT GetBackBuffer(UINT, UINT, int, IDirect3DSurface9** out){ if(out)*out=new IDirect3DSurface9(); return S_OK;} };
struct IDirect3DTexture9 : D3DReleasable { virtual ~IDirect3DTexture9() = default; HRESULT LockRect(UINT, D3DLOCKED_RECT*, const RECT*, DWORD){return E_FAIL;} HRESULT UnlockRect(UINT){return S_OK;} };
