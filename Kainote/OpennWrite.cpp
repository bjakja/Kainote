...
OpenWrite::OpenWrite(const wxString &fileName, bool clear)
{
	...
	if (!file.Exists(fileName)){
		if (!file.Create(fileName, false, wxS_DEFAULT)){ KaiLog(_(L"Nie można utworzyć pliku.")); }
	}
	else{
		if (!file.Open(fileName, (clear) ? wxFile::write : wxFile::write_append, wxS_DEFAULT)){ KaiLog(_(L"Nie można otworzyć pliku.")); };
	}
	isfirst = clear;
}
...
void OpenWrite::FileWrite(const wxString &fileName, const wxString &textfile, bool utf)
{
	...
	if (fname.FileExists() && !fname.IsFileReadable()){
		KaiLog(_(L"Nie można odczytać pliku."));
		return;
	}
...
{
	if (!file.IsOpened()){ KaiLog(_(L"Plik nie został otwarty.")); return; }
	if (isfirst){
		wchar_t bom = 0xFEFF;
		if (!file.Write(wxString(bom) + parttext/*,wxConvUTF8*/)){ KaiLog(_(L"Nie można zapisać do pliku.")); };
		isfirst = false;
		return;
	}
	if (!file.Write(parttext/*,wxConvUTF8*/)){ KaiLog(_(L"Nie można zapisać do pliku.")); };
}
