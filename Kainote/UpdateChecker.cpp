...
namespace
{
	...
	public:
		UpdateAvailableDialog(wxWindow *parent, const ReleaseInfo &release)
			: KaiDialog(parent, -1, _("Dostępna jest nowa wersja"))
		{
			...
			sizer->Add(new KaiStaticText(this, -1,
				wxString::Format(_("Masz wersję %s, dostępna jest %s"),
			...
			autoCheck = new KaiCheckBox(this, -1, _("Automatycznie sprawdzaj aktualizacje"));
			autoCheck->SetValue(Options.GetBool(UPDATER_AUTO_CHECK));
			sizer->Add(autoCheck, 0, wxEXPAND | wxALL, 4);

			stableOnly = new KaiCheckBox(this, -1, _("Tylko wersje stabilne"));
			...
			MappedButton *open = new MappedButton(this, 20001, _("Otwórz stronę pobierania"));
			MappedButton *later = new MappedButton(this, 20002, _("Przypomnij za tydzień"));
			MappedButton *close = new MappedButton(this, wxID_CLOSE, _("Zamknij"));
	...
	void Check(wxWindow *parent, bool interactive)
	{
		...
		if (!request.IsOk()) {
			if (interactive)
				KaiMessageBox(_("Nie można sprawdzić aktualizacji"), _("Aktualizacja"));
			return;
		}
		...
		parent->Bind(wxEVT_WEBREQUEST_STATE, [parent, interactive](wxWebRequestEvent &evt) {
			...
			{
				if (interactive)
					KaiMessageBox(_("Masz najnowszą wersję programu"), _("Aktualizacja"));
				return;
			}
