	  ...
	  m_width(info.GetWidth())
{
	...
	case wxGRADIENT_LINEAR:
		...
		m_stippleBrush->CreateLinearGradientBrush(
								...
								m_penInfo.GetStops(),
								// NULL: nothing in Kainote sets a gradient matrix.
								NULL);
		break;
	...
	case wxGRADIENT_RADIAL:
		...
		m_stippleBrush->CreateRadialGradientBrush(
								...
								m_penInfo.GetRadius(),
								m_penInfo.GetStops(),
								// NULL: wx's wxGraphicsPenInfo hands out a wxGraphicsMatrix, not the
								// wxD2DMatrixData this takes, and nothing in Kainote sets a gradient
								// matrix (no LinearGradient/RadialGradient call), so the pen's is the
								// default null one -- which is the case this NULL already handles.
								NULL);
		break;
	}
}
