...
chapters(nullptr)
{
	...
	if (FAILED(hr)){ KaiLog(_(L"Nie można zainicjalizować COM")); }
}
...
{
	PTR(InitializeGraph(), _(L"Błąd inicjalizacji Direct Show"));
	...
	HR(m_pGraph->AddSourceFilter(sFileName.wc_str(), L"Source Filter", &pSource.obj), _(L"Filtr źródła nie został dodany"));
	...
	*/
	...
	HR(m_pGraph->AddFilter(frend.obj, L"Kainote video Renderer"), _(L"Nie można dodać renderera wideo"));
	...
	//
	HR(CoCreateInstance(CLSID_DSoundRender, nullptr, CLSCTX_INPROC, IID_IBaseFilter, (LPVOID*)&pAudioRenderer.obj), _(L"Nie można utworzyć instancji renderera dźwięku"));

	HR(m_pGraph->AddFilter(pAudioRenderer.obj, L"Direct Sound Renderer"), _(L"Nie można dodać renderera Direct Sound"));
	...
	if (vobsub){
		...
		if (pVobsub.obj){
			...
			HR(hr = pSource->EnumPins(&pEnum.obj), _(L"Nie można wyliczyć pinów źródła"));
			...
			HR(hr = pVobsub->EnumPins(&pEnum.obj), _(L"Nie można wyliczyć pinów źródła"));
			while (S_OK == pEnum->Next(1, &pPin.obj, nullptr))
			{
		...
		HR(pSource->EnumPins(&Enumsrc.obj), _(L"Nie można wyliczyć pinów źródła"));
		HR(frend->EnumPins(&Enumrend.obj), _(L"Nie można wyliczyć pinów renderera"));
		HR(pAudioRenderer->EnumPins(&Enumaud.obj), _(L"Nie można wyliczyć pinów dsound"));
		...
		HR(Enumrend->Next(1, &rpin.obj, 0), _(L"Nie można pobrać pinu renderera"));
		HR(Enumaud->Next(1, &apin.obj, 0), _(L"Nie można pobrać pinu dsound"));
		/*PIN_INFO pinfo;
		...
		while (Enumsrc->Next(1, &spin.obj, 0) == S_OK)
		{
			...
			HR(mtypes->Next(1, &info, 0), _(L"Brak informacji o rodzaju ścieżki"));

			if (info->majortype == MEDIATYPE_Video){

				HR(m_pGraph->Connect(spin.obj, rpin.obj), _(L"Nie można połączyć pinu źródła z rendererem wideo"));
			}
			else if (info->majortype == MEDIATYPE_Audio){

				HR(m_pGraph->Connect(spin.obj, apin.obj), _(L"Nie można połączyć pinu źródła z rendererem audio"));
			}
			else if (info->majortype == MEDIATYPE_Stream){

				HR(m_pGraph->Connect(spin.obj, rpin.obj), _(L"Nie można połączyć pinu źródła z rendererem audio1"));
				SAFE_RELEASE(rpin.obj);
				HR(spin.obj->ConnectedTo(&rpin.obj), _(L"Nie można znaleźć połączonego pinu źródła"));
				PIN_INFO pinfo;
				HR(rpin.obj->QueryPinInfo(&pinfo), _(L"Nie można pobrać informacji o pinie splittera"));
				SAFE_RELEASE(spin.obj);
				SAFE_RELEASE(Enumrend.obj);
				HR(pinfo.pFilter->EnumPins(&Enumrend.obj), _(L"Nie można wyliczyć pinów splittera"));
				...
				while (Enumrend->Next(1, &spin.obj, 0) == S_OK)
				{
					...
					if (info->majortype == MEDIATYPE_Audio){
						HR(m_pGraph->Connect(spin.obj, apin.obj), _(L"Nie można połączyć pinu źródła z rendererem wideo2"));
						break;
					}
					//DeleteMediaType(info);info=0;
					SAFE_RELEASE(mtypes.obj);
	...
	if (FAILED(pSource->QueryInterface(IID_IAMStreamSelect, (void**)&stream))){
		...
		Selfdest<IEnumPins> pEnum;
		HR(hr = pSource->EnumPins(&pEnum.obj), _(L"Nie można wyliczyć pinów źródła"));
		HR(pEnum->Next(1, &spin.obj, nullptr), _(L"Nie można pobrać pinu źródła"));
		spin->ConnectedTo(&strpin.obj);
		PIN_INFO pinfo;
		HR(strpin.obj->QueryPinInfo(&pinfo), _(L"Nie można pobrać informacji o pinie splittera"));
...
bool DShowPlayer::InitializeGraph()
{
	...
	HR(hr = CoCreateInstance(CLSID_FilterGraph, nullptr, CLSCTX_INPROC_SERVER, IID_IGraphBuilder, (void**)&m_pGraph), _(L"Nie można stworzyć interfejsu filtrów"));

	// Query for graph interfaces. (These interfaces are exposed by the graph
	// manager regardless of which filters are in the graph.)
	HR(hr = m_pGraph->QueryInterface(IID_IMediaControl, (void**)&m_pControl), _(L"Nie można stworzyć kontrolera"));
	HR(hr = m_pGraph->QueryInterface(IID_IMediaSeeking, (void**)&m_pSeek), _(L"Nie można stworzyć interfejsu szukania"));
	HR(hr = m_pGraph->QueryInterface(IID_IBasicAudio, (void**)&m_pBA), _(L"Nie można stworzyć interfejsu audio"));
...
bool DShowPlayer::EnumFilters(Menu *menu)
{
	...
	FILTER_INFO fi;
	int numfilter = 0;
	HR(m_pGraph->EnumFilters(&efilters.obj), _(L"Nie można wyliczyć filtrów"));
	while (S_OK == efilters->Next(1, &bfilter, 0))
	{
		bfilter->QueryInterface(__uuidof(ISpecifyPropertyPages), (void**)&ppages);
		HR(bfilter->QueryFilterInfo(&fi), _(L"Nie można pobrać nazwy filtra"));
...
bool DShowPlayer::FilterConfig(wxString name, int idx, wxPoint pos)
{
	...
	HR(m_pGraph->FindFilterByName(name.wc_str(), &bfilter.obj), _(L"Nie można wyliczyć filtrów"));
	bfilter->QueryInterface(__uuidof(ISpecifyPropertyPages), (void**)&ppages.obj);
	if (ppages.obj == 0){ return false; }
	HR(ppages->GetPages(&caGUID), _(L"Nie można pobrać konfiguracji filtra"));
