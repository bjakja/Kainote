...
bool RendererVideo::InitDX()
{
	...
	if (!m_D3DObject){
		...
		PTR(m_D3DObject, _(L"Nie można utworzyć obiektu Direct3D"));
	}
	else{
		Clear(false);
	}
	...
	if (m_D3DDevice){
		...
		if (FAILED(hr)){
			KaiLogSilent(L"Video: " + _(L"Nie można zresetować Direct3D"));
			return false;
		}
	}
	else{
		...
		if (FAILED(hr)){
			HR(m_D3DObject->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, m_HWND,
				...
				D3DCREATE_FPU_PRESERVE /*| D3DCREATE_PUREDEVICE*/, &d3dpp, &m_D3DDevice),
				_(L"Nie można utworzyć urządzenia D3D9"));
		}
	}
	...
	HR(m_D3DDevice->SetTransform(D3DTS_PROJECTION, &matOrtho), _(L"Nie można ustawić macierzy projekcji"));
	HR(m_D3DDevice->SetTransform(D3DTS_WORLD, &matIdentity), _(L"Nie można ustawić macierzy świata"));
	HR(m_D3DDevice->SetTransform(D3DTS_VIEW, &matIdentity), _(L"Nie można ustawić macierzy widoku"));
	...
	HR(D3DXCreateFontW(m_D3DDevice, pixelSize.y, pixelSize.x, FW_BOLD, 0, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLEARTYPE_QUALITY,
		DEFAULT_PITCH | FF_DONTCARE, L"Tahoma", &m_D3DFont), _(L"Nie można stworzyć czcionki D3DX"));
	HR(D3DXCreateFontW(m_D3DDevice, pixelSize.y, pixelSize.x, FW_BOLD, 0, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLEARTYPE_QUALITY,
		DEFAULT_PITCH | FF_DONTCARE, L"Tahoma", &m_D3DCalcFont), _(L"Nie można stworzyć czcionki D3DX"));
	HR(D3DXCreateLine(m_D3DDevice, &m_D3DLine), _(L"Nie można stworzyć linii D3DX"));
...
void RendererVideo::SetZoom(float percent, const wxPoint& mousePos)
{
	...
	if (percent != -1) {
		...
		float xpar = x / percent;
		float ypar = y / percent;
		...
		if ((m_ZoomPercent == 1.f && m_HasZoom) || m_ZoomRect.width < 1) {
			float npercent = Options.GetInt(VIDEO_ZOOM_PERCENT) / 100.f;
			npercent = (npercent < 1.f || npercent > 11.f)? 2.f : npercent;
			float x = m_BackBufferRect.right - m_BackBufferRect.left;
			float y = m_BackBufferRect.bottom - m_BackBufferRect.top;
			float xpar = x / npercent;
			float ypar = y / npercent;
...
void RendererVideo::Zoom(const wxSize &size)
{
	...
	m_MainStreamRect.right = (m_ZoomRect.width - m_BackBufferRect.left) / videoToScreenXX;
	m_MainStreamRect.bottom = (m_ZoomRect.height - m_BackBufferRect.top) / videoToScreenYY;
	m_ZoomPercent = size.x / (m_ZoomRect.width - m_ZoomRect.x/* + backBufferRect.left*/);
	if (videoControl->m_IsFullscreen){ UpdateRects(false); }
	if (m_Visual){
		SetVisualZoom();
		if (m_Visual && (m_Visual->Visual < CLIPRECT || m_Visual->Visual > VECTORDRAW)){
