	...
	, changedForeground(false)
{
	...
	if (toolTip != emptyString){ SetToolTip(toolTip); }
...
{
	if (Window >= 0){
		wxString toolTip = (_toolTip == emptyString) ? GetToolTipText().BeforeFirst(L'(').BeforeFirst(L'\n').Trim() : _toolTip;
		wxString desc = name;
...
void MappedButton::OnMouseEvent(wxMouseEvent &event)
{
	...
	if (Window >= 0 && event.LeftDown() && (event.ShiftDown() || (twoHotkeys && event.ControlDown()))){
		...
		Hkeys.OnMapHkey(GetId(), emptyString, this, Window, false);
		SetToolTip();
		SetFocus();
		return;
	}
