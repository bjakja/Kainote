...
void RotationXY::DrawVisual(int time)
{
	...
	HRN(device->SetTransform(D3DTS_PROJECTION, &matOrtho), _("Cannot set matrix projection"));
	HRN(device->SetTransform(D3DTS_WORLD, &matIdentity), _("Cannot set world matrix"));
	HRN(device->SetTransform(D3DTS_VIEW, &matIdentity), _("Cannot set view matrix"));
	D3DXVECTOR2 v2[4];
	v2[0].x = org.x - 10.0f;
	v2[0].y = org.y;
	v2[1].x = org.x + 10.0f;
