...
//out: m128_1 = avg(m128_1.u8[0],m128_1.u8[1]) 
//              0 
//              avg(...) 
//              0 
//              ...
#define AVERAGE_4_PIX_INTRINSICS_0(m128_1, m128_tmp) \
    m128_tmp = _mm_slli_epi16(m128_1, 8); \
    m128_1 = _mm_srli_epi16(m128_1, 8); \
    m128_tmp = _mm_srli_epi16(m128_tmp, 8); \
    m128_1 = _mm_avg_epu8(m128_1, m128_tmp);
...
static __forceinline void mix_16_y_p010_P210_sse2(BYTE* dst, const BYTE* src, const BYTE* src_alpha)
{
    //important!
...
static __forceinline void mix_16_uv_p210_sse2(BYTE* dst, const BYTE* src, const BYTE* src_alpha, int pitch)
{
    // 4:2:2 only horizontal subsampling
    ...
    __m128i tmp;
    AVERAGE_4_PIX_INTRINSICS_0(alpha, tmp); // average two horizontally
...
static void mix_16_uv_p210_c(BYTE* dst, const BYTE* src, const BYTE* src_alpha, int pitch)
{
    WORD* dst_word = reinterpret_cast<WORD*>(dst);
    for (int i = 0; i < 8; i++, src_alpha += 2, src += 2, dst_word += 2)
    {
        // only horizontal subsampling
        unsigned int ia = (src_alpha[0] + src_alpha[1] + 1) / 2;
        if (ia != 0xFF)
        {
            int tmp = (((dst_word[0]) * ia) >> 8) + (src[0] << 8);
            if (tmp > 0xffff) tmp = 0xffff;
...
static __forceinline void subsample_422_and_interlace_2_line_c(BYTE* dst, const BYTE* u, const BYTE* v, int w, int pitch, int last_src_id = 0)
{
    const BYTE* end = u + w;
    BYTE last_u = u[last_src_id];
    BYTE last_v = v[last_src_id];
    for (; u < end; dst += 2, u += 2, v += 2)
    {
        dst[0] = u[0];
        int tmp1 = u[1];
...
// @w : w % 16 must == 0!
static __forceinline void subsample_422_and_interlace_2_line_sse2(BYTE* dst, const BYTE* u, const BYTE* v, int w, int pitch, int last_src_id = 0)
{
    const BYTE* end_mod16 = u + (w & ~15);

    __m128i u_last = _mm_cvtsi32_si128(u[last_src_id] << 8);
    __m128i v_last = _mm_cvtsi32_si128(v[last_src_id] << 8);
    for (; u < end_mod16; dst += 16, u += 16, v += 16)
    {
        __m128i u_1 = _mm_load_si128(reinterpret_cast<const __m128i*>(u));
        __m128i v_1 = _mm_load_si128(reinterpret_cast<const __m128i*>(v));
...
static __forceinline void mix_uv_yv16_c(byte* dst, int w, const byte* src, const byte* am, int src_pitch, int last_src_id = 0)
{
    int last_alpha = am[last_src_id];
    int last_sub = src[last_src_id];
    const BYTE* end = src + w;
    for (; src < end; src += 2, am += 2, dst++)
    {
        int ia = am[0];
...
//0<=w15<=15 && w15%2==0
static __forceinline void mix_uv_yv16_c2(byte* dst, int w15, const byte* src, const byte* am, int src_pitch, int last_src_id = 0)
{
    ASSERT(w15 >= 0 && w15 <= 15 && (w15 & 1) == 0);
...
// am[last_src_id] valid && w&15=0
static __forceinline void mix_uv_yv16_sse2(byte* dst, int w00, const byte* src, const byte* am, int src_pitch, int last_src_id = 0)
{
    ASSERT((((2 * (intptr_t)dst) | w00 | (intptr_t)src | (intptr_t)am | src_pitch) & 15) == 0);

    __m128i last_src = _mm_cvtsi32_si128(src[last_src_id] << 8);
    __m128i last_alpha = _mm_cvtsi32_si128(am[last_src_id] << 8);
    const BYTE* end_mod16 = src + (w00 & ~15);
    for (; src < end_mod16; src += 16, am += 16, dst += 8)
    {
        __m128i zero = _mm_setzero_si128();
...
static __forceinline void mix_uv_p210_c(BYTE* dst, int w, const BYTE* src, const BYTE* am, int src_pitch, int last_src_id = 0)
{
...
//0<=w15<=15
static __forceinline void mix_uv_p210_c2(BYTE* dst, int w15, const BYTE* src, const BYTE* am, int src_pitch, int last_src_id = 0)
{
    ASSERT(w15 >= 0 && w15 <= 15 && (w15 & 1) == 0);
    int last_alpha = am[last_src_id];
    WORD* dst_word = reinterpret_cast<WORD*>(dst);
...
#ifdef XY_UNIT_TEST 
#  define  mix_uv_p210_c2_CLIP(tmp) tmp ^= (tmp^0xffff)&((0xffff-tmp)>>31);/*if(tmp>0xffff) tmp = 0xffff;*/
#else
#  define  mix_uv_p210_c2_CLIP(tmp) 
#endif

    switch (w15)
    {
    case 14:
#define mix_uv_p210_c2_mix_2 \
    int ia = am[0];\
    int tmp2 = am[1];\
    last_alpha = (last_alpha + tmp2 + 1)/2;\
    ia = (ia + last_alpha + 1)/2;\
...
// am[last_src_id] valid && w&15=0
static __forceinline void mix_uv_p210_sse2(BYTE* dst, int w00, const BYTE* src, const BYTE* am, int src_pitch, int last_src_id = 0)
{
    ASSERT((((intptr_t)dst | w00 | (intptr_t)src | (intptr_t)am | src_pitch) & 15) == 0);
    __m128i last_alpha = _mm_cvtsi32_si128((am[last_src_id] + am[src_pitch + last_src_id] + 1) << 7);
    const BYTE* end_mod16 = src + w00;
