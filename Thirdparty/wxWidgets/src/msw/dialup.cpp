...
struct  wxRasThreadData
{
    ...
    class  wxDialUpManagerMSW *dialUpManager;  // the owner
};
...
class  wxDialUpManagerMSW : public wxDialUpManager
{
public:
    // ctor & dtor
    wxDialUpManagerMSW();
...
exit:
        if ( funcName )
        {
            wxLogError(_("The version of remote access service (RAS) installed on this machine is too old, please upgrade (the following required function is missing: %s)."),
                       funcName);
            m_dllRas.Unload();
            return;
        }
