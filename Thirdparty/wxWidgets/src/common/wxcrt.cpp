...
WXDLLIMPEXP_BASE size_t wxMB2WC(wchar_t *buf, const char *psz, size_t n)
{
  ...
  // C99) extension which allows to call mbs(r)towcs() with nullptr output pointer
...
WXDLLIMPEXP_BASE size_t wxWC2MB(char *buf, const wchar_t *pwz, size_t n)
{
  ...
  if (buf) {
    if (!n || !*pwz) {
      // glibc2.1 chokes on nullptr input
      if (n) *buf = '\0';
      return 0;
    }
#ifdef HAVE_WCSRTOMBS
...
static int vswscanf(const wchar_t *ws, const wchar_t *format, va_list argptr)
{
    ...
    wxCHECK_MSG( wxStrstr(format, wxT("%s")) == nullptr, -1,
                 wxT("incomplete vswscanf implementation doesn't allow %s") );
    wxCHECK_MSG( wxStrstr(format, wxT("%c")) == nullptr, -1,
