...
bool wxImage::Create( int width, int height, unsigned char* data, bool static_data )
{
    ...
    wxCHECK_MSG( data, false, wxT("nullptr data in wxImage::Create") );
...
void wxImage::SetRGB( const wxRect& rect_, unsigned char r, unsigned char g, unsigned char b )
{
    ...
    unsigned char *data wxDUMMY_INITIALIZE(nullptr);
...
bool wxImage::LoadFile( wxInputStream& stream, wxBitmapType type, int index )
{
    ...
    if ( type == wxBITMAP_TYPE_ANY )
    {
        if ( !stream.IsSeekable() )
        {
            ...
            wxLogError(_("Can't automatically determine the image format for non-seekable input."));
            return false;
        }
...
{
    wxCHECK_MSG( x && y, wxIMAGE_RESOLUTION_NONE, wxT("nullptr pointer") );
